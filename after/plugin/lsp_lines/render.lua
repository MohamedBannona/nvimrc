...
function M.show(namespace, bufnr, diagnostics, opts)
    ...
    for _, diagnostic in ipairs(diagnostics) do
        ...
        if diagnostic.lnum ~= prev_lnum then
            table.insert(stack, {
                SPACE,
                string.rep(" ", distance_between_cols(bufnr, diagnostic.lnum, 0, diagnostic.col)),
            })
        elseif diagnostic.col ~= prev_col then
            ...
            table.insert(stack, {
                ...
                string.rep(
                    " ",
                    distance_between_cols(bufnr, diagnostic.lnum, prev_col + 1, diagnostic.col) - 1
                ),
            })
        else
    ...
    local chars = config.box_drawing_characters
